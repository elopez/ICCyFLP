...
\begin{comment}
    ...
    Aquí habría una prueba si hubiera podido realizarla :/
    \end{proof}
\end{comment}
\end{homeworkProblem}
...
\begin{homeworkProblem}[23]
    ...
    El algoritmo de Deutsch comienza aplicando compuertas Hadamard a
    $\ket{0}$ y $\ket{1}$:
    \[\ket{01} \xrightarrow{H(1,2)} \ket{+} \otimes \frac{1}{\sqrt{2}}\left(\ket{0} - \ket{1}\right) = \frac{1}{\sqrt{2}} \left(\ket{+0} - \ket{+1}\right)\]
    Luego se aplica la compuerta $U_{id}$
    \[\begin{aligned}
    U_{id}\left(\frac{1}{\sqrt{2}} \left(\ket{+0} - \ket{+1}\right)\right) &= \frac{1}{\sqrt{2}} \left(U_{id}\ket{+0} - U_{id}\ket{+1}\right) \\
    &= \frac{1}{\sqrt{2}} \left(\frac{1}{\sqrt{2}} \left(\ket{0, \textrm{id } 0} + \ket{1, \textrm{id } 1}\right)-\frac{1}{\sqrt{2}} \left(\ket{0, 1 \oplus \textrm{id } 0} + \ket{1, 1 \oplus \textrm{id } 1}\right)\right) \\
    &= \frac{1}{2} \left(\ket{00} + \ket{11} - \ket{01} - \ket{10}\right) \\
    &= \left(\frac{\ket{0} - \ket{1}}{\sqrt{2}}\right) \otimes \left(\frac{\ket{0} - \ket{1}}{\sqrt{2}}\right) \\
    &= \ket{--}
    \end{aligned}\]
    Finalmente, se aplica Hadamard al primer qubit, resultando $\ket{1}$
    con probabilidad 1. Esto implica que la función identidad no es
    constante.
\end{homeworkProblem}
...
\begin{homeworkProblem}[25]
    ...
    Debido a que el algoritmo de Grover funciona sobre cantidades
    $N = 2^{n}$, primero se deberá completar el arreglo con nuevos
    elementos, por lo que para el análisis de este ejercicio se usa
    $N = 2^{\ceil*{\log_{2}1000000}} = 2^{20}$.

    \begin{enumerate}[label=(\alph*)]
        \item El número óptimo de iteraciones para buscar un elemento
        usando el algoritmo de Grover está dado por la fórmula
        $k = \floor*{\frac{\pi}{4\arcsin \sqrt{\frac{1}{2^{n}}}}}$. En
        particular, $k = 804$ para $n = 20$.
        \item Una cota superior de la probabilidad de error con el
        número óptimo de iteraciones es $\frac{1}{2^{n}}$. Con $n = 20$
        esto resulta ser $0.0009765625$.
        \item Para encontrar un elemento en una lista no ordenada usando
        un algoritmo clásico, se necesitarán revisar $\frac{N}{2}$
        elementos en el caso promedio. Al no ser necesario completar el
        arreglo para tener una cantidad $2^{n}$ de elementos, esto
        equivale a $500000$ pasos en este caso.
    \end{enumerate}
\end{homeworkProblem}
